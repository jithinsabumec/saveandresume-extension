 ...
 * @public
 */
const index_esm_getDefaultAppConfig = () => getDefaults()?.config;
/**
 ...
 * available.
 */
function index_esm_isBrowser() {
    return typeof window !== 'undefined' || index_esm_isWebWorker();
}
/**
 * Detect Web Worker context.
 */
function index_esm_isWebWorker() {
 ...
 */
class index_esm_ComponentContainer {
    constructor(name) {
        this.name = name;
 ...
 * @internal
 */
function index_esm_isFirebaseServerApp(obj) {
...
async function _performApiRequest(auth, method, path, request, customErrorMap = {}) {
    return _performFetchWithErrorHandling(auth, customErrorMap, async () => {
        ...
        const query = index_esm_querystring({
...
class UserImpl {
    ...
    async delete() {
        if (index_esm_isFirebaseServerApp(this.auth.app)) {
...
class AuthImpl {
    ...
    async initializeCurrentUser(popupRedirectResolver) {
        if (index_esm_isFirebaseServerApp(this.app)) {
    ...
    async _getAppCheckToken() {
        if (index_esm_isFirebaseServerApp(this.app) && this.app.settings.appCheckToken) {
...
async function _reauthenticate(user, credential, bypassAuthState = false) {
    const { auth } = user;
    if (index_esm_isFirebaseServerApp(auth.app)) {
        return Promise.reject(_serverAppCurrentUserOperationNotSupportedError(auth));
    }
...
function onIdTokenChanged(auth, nextOrObserver, error, completed) {
    return index_esm_getModularInstance(auth).onIdTokenChanged(nextOrObserver, error, completed);
}
/**
...
function beforeAuthStateChanged(auth, callback, onAbort) {
    return index_esm_getModularInstance(auth).beforeAuthStateChanged(callback, onAbort);
}
/**
 ...
 * @public
 */
function index_1970027f_signOut(auth) {
    return index_esm_getModularInstance(auth).signOut();
}
/**
 * Revokes the given access token. Currently only supports Apple OAuth access tokens.
 *
 ...
 * limitations under the License.
 */
const index_1970027f_DB_NAME = 'firebaseLocalStorageDb';
const index_1970027f_DB_VERSION = 1;
...
function _deleteDatabase() {
    const request = indexedDB.deleteDatabase(index_1970027f_DB_NAME);
    return new DBPromise(request).toPromise();
}
function _openDatabase() {
    const request = indexedDB.open(index_1970027f_DB_NAME, index_1970027f_DB_VERSION);
    return new Promise((resolve, reject) => {
...
async function _getRedirectUrl(auth, provider, authType, redirectUrl, eventId, additionalParams) {
    ...
    return `${getHandlerBase(auth)}?${index_esm_querystring(paramsDict).slice(1)}${appCheckTokenFragment}`;
}
...
var index_1970027f_version = "1.11.1";
...
function registerAuth(clientPlatform) {
    ...
    registerVersion(index_1970027f_name, index_1970027f_version, getVersionForPlatform(clientPlatform));
    // BUILD_TARGET will be replaced by values like esm, cjs, etc during the compilation
    registerVersion(index_1970027f_name, index_1970027f_version, 'esm2020');
}
...
(function () {
    ...
    async function signOut() {
        const authInstance = requireAuthInstance();
        await index_1970027f_signOut(authInstance);
